...
func init() {
	orm.RegisterModel(new(PedidoItems))
}

// AddPedidoItems insert a new PedidoItems into database and returns
// last inserted Id on success.
func AddPedidoItems(m *PedidoItems) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetPedidoItemsById retrieves PedidoItems by Id. Returns error if
// Id doesn't exist
func GetPedidoItemsById(id int) (v *PedidoItems, err error) {
	o := orm.NewOrm()
	v = &PedidoItems{Id: id}
...
// GetAllPedidoItems retrieves all PedidoItems matches certain condition. Returns empty list if
// no records exist
...
// UpdatePedidoItems updates PedidoItems by Id and returns error if
// the record to be updated doesn't exist
func UpdatePedidoItemsById(m *PedidoItems) (err error) {
	o := orm.NewOrm()
	v := PedidoItems{Id: m.Id}
...
// DeletePedidoItems deletes PedidoItems by Id and returns error if
// the record to be deleted doesn't exist
func DeletePedidoItems(id int) (err error) {
	o := orm.NewOrm()
	v := PedidoItems{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&PedidoItems{Id: id}); err == nil {
