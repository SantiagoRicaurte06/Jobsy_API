...
type Postulaciones struct {
	Id                 int                 `orm:"column(id_postulaciones);pk"`
	...
	IdModalidadDisp    *ModalidadesTrabajo `orm:"column(id_modalidad_disp);rel(fk)"`
	IdTurnoPostulacion *Turnos             `orm:"column(id_turno_postulacion);rel(fk)"`
	NotaDisponibilidad string              `orm:"column(nota_disponibilidad);null"`
	IdFormaCobro       *Tipos              `orm:"column(id_forma_cobro);rel(fk)"`
...
func GetAllPostulaciones(query map[string]string, fields []string, sortby []string, order []string,
	...
	o := orm.NewOrm()
	qs := o.QueryTable(new(Postulaciones))
	// query k=v
	for k, v := range query {
