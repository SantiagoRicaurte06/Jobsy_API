...
type PerfilesEmpleado struct {
	Id                 int       `orm:"column(id_perfiles_empleado);pk;auto"`
	IdUsuarios         int       `orm:"column(id_usuarios)"`
	Bio                string    `orm:"column(bio);null"`
	IdNivelExperiencia int       `orm:"column(id_nivel_experiencia)"`
	...
	IdTurnoPreferido   int       `orm:"column(id_turno_preferido)"`
...
func GetPerfilesEmpleadoById(id int) (v *PerfilesEmpleado, err error) {
	...
	if err = o.Read(v); err == nil {
		...
		o.LoadRelated(v, "IdNivelExperiencia")
		o.LoadRelated(v, "IdModalidadTrabajo")
		o.LoadRelated(v, "IdTurnoPreferido")
		return v, nil
	}
	return nil, err
}
...
func GetAllPerfilesEmpleado(query map[string]string, fields []string, sortby []string, order []string,
	...
	o := orm.NewOrm()
	qs := o.QueryTable(new(PerfilesEmpleado)).RelatedSel()
	// query k=v
	for k, v := range query {
