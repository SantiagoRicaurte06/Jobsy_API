...
type Contrataciones struct {
	Id                 int                 `orm:"column(id_contrataciones);pk:auto"`
	...
	IdModalidad        int 				   `orm:"column(id_modalidad)"`
...
func GetAllContrataciones(query map[string]string, fields []string, sortby []string, order []string,
	...
	o := orm.NewOrm()
	qs := o.QueryTable(new(Contrataciones)).RelatedSel()
	// query k=v
	for k, v := range query {
