...
// @Success 201 {int} models.Postulaciones
// @Failure 403 body is empty
// @router / [post]
func (c *PostulacionesController) Post() {
	var v models.Postulaciones
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if _, err := models.AddPostulaciones(&v); err == nil {
...
func (c *PostulacionesController) GetOne() {
	...
	v, err := models.GetPostulacionesById(id)
	if err != nil {
		logs.Error(err)
...
func (c *PostulacionesController) GetAll() {
	...
	l, err := models.GetAllPostulaciones(query, fields, sortby, order, offset, limit)
	if err != nil {
		logs.Error(err)
...
func (c *PostulacionesController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.Postulaciones{Id: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdatePostulacionesById(&v); err == nil {
...
func (c *PostulacionesController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := models.DeletePostulaciones(id); err == nil {
		c.Ctx.Output.SetStatus(200)
