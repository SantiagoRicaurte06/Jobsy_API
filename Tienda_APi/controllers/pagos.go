...
func (c *PagosController) GetOne() {
	...
	if err != nil {
		...
		c.Data["json"] = map[string]interface{}{
			"success": false,
			"status":  400,
			"message": "El id del pago debe ser un número entero válido",
			"data":    nil,
		}
		c.ServeJSON()
		return
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdatePagosById(&v); err == nil {
			...
			c.Data["json"] = map[string]interface{}{
				"success": false,
				"status":  400,
				"message": "No se pudo actualizar el pago, verifique que el id exista y que los datos enviados sean correctos",
				"data":    nil,
			}
		}
	} else {
...
func (c *PagosController) Delete() {
	...
	if err := models.DeletePagos(id); err == nil {
		...
		c.Data["json"] = map[string]interface{}{
			"success": false,
			"status":  400,
			"message": "No se pudo eliminar el pago, es posible que tenga transacciones asociadas o que el id no exista",
			"data":    nil,
		}
	}
	c.ServeJSON()
