...
func init() {
	orm.RegisterModel(new(SaldoJobsy))
}

// AddSaldoJobsy insert a new SaldoJobsy into database and returns
// last inserted Id on success.
func AddSaldoJobsy(m *SaldoJobsy) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetSaldoJobsyById retrieves SaldoJobsy by Id. Returns error if
// Id doesn't exist
func GetSaldoJobsyById(id int) (v *SaldoJobsy, err error) {
	o := orm.NewOrm()
	v = &SaldoJobsy{Id: id}
...
// GetAllSaldoJobsy retrieves all SaldoJobsy matches certain condition. Returns empty list if
// no records exist
...
// UpdateSaldoJobsy updates SaldoJobsy by Id and returns error if
// the record to be updated doesn't exist
func UpdateSaldoJobsyById(m *SaldoJobsy) (err error) {
	o := orm.NewOrm()
	v := SaldoJobsy{Id: m.Id}
...
// DeleteSaldoJobsy deletes SaldoJobsy by Id and returns error if
// the record to be deleted doesn't exist
func DeleteSaldoJobsy(id int) (err error) {
	o := orm.NewOrm()
	v := SaldoJobsy{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&SaldoJobsy{Id: id}); err == nil {
